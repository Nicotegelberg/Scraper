...
class MetacriticSpider(scrapy.Spider):
    ...
    #The urls for the platforms we want to scrape the product from
    start_urls = [
    ...
    def parse(self, response):
        for game_link in response.css('a.product-title::attr(href)').getall():
            yield scrapy.Request(url=f"https://www.bol.com{game_link}", callback=self.parse_game) #callback
    
    #Go through all the pages of the platforms
...
def parse_game(self, response):
    title = response.css('h1.page-heading span[data-test="title"]::text').get()
    platform = response.xpath('normalize-space(.//div[@class="specs__row"][2]/div[@class="specs__value"]/text())').get()
    price = response.css("span[data-test='price']::text").get().strip()
